...
//Database path
    let dbPath: String = "GDRB MyordersProduct.sqlite"
   ...
   //create a table in sqlite
    func createTable() {
        let createTableString = "CREATE TABLE IF NOT EXISTS MyOrdersDetails(productid INTEGER PRIMARY KEY AUTOINCREMENT,productname TEXT,productprice TEXT,productcategory TEXT,productDiscription TEXT,productRating TEXT,productImage BLOB );"
        var createTableStatement: OpaquePointer? = nil
        if sqlite3_prepare_v2(db, createTableString, -1, &createTableStatement, nil) == SQLITE_OK
        {
            if sqlite3_step(createTableStatement) == SQLITE_DONE{
            
                print("myorders table created.")
            } else {
                print("myorders table could not be created.")
            }
        } else {
    ...
    func insert( productname:String, productprice:String,productcategory:String,productDiscription:String,productRating:String,image:UIImage){
        ...
        let insertStatementString = "INSERT INTO MyOrdersDetails (productname, productprice, productcategory, productDiscription, productRating, productImage ) VALUES (?, ?, ?, ?, ?, ?);"
    ...
    func retrieveProducts(withCategory category: String) -> [MyOrdersDetails] {
        var products: [MyOrdersDetails] = []
        
        let query = """
            SELECT * FROM MyOrdersDetails
        ...
        if sqlite3_prepare_v2(db, query, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                ...
                if let imageData = imageData {
                    ...
                    let product = MyOrdersDetails(productname: productName, productprice: productPrice, productcategory: productCategory, productid: Int(productId), productDiscription: productDescription, productRating: productRating, image: image!)
    ...
    func read() -> [MyOrdersDetails] {
        let queryStatementString = "SELECT * FROM MyOrdersDetails;"
        var queryStatement: OpaquePointer? = nil
        var HomeProduct : [MyOrdersDetails] = []
        ...
        if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                ...
                if let imageData = imageData {
                    ...
                    let image = UIImage(data: data)
                    // Use the image as needed
                    HomeProduct.append(MyOrdersDetails(productname: productname, productprice: productprice, productcategory: productcategory, productid: Int(productid), productDiscription: productDiscription, productRating: productRating, image: image!))
                     print("Query Result:")
    ...
    func retrieveProductsDetails() -> [MyOrdersDetails] {
        var products: [MyOrdersDetails] = []
        
        let query = "SELECT * FROM MyOrdersDetails"
        var queryStatement: OpaquePointer?
