...
//Database path
    let dbPath: String = "GDRB WishList.sqlite"
   ...
   //create a table in sqlite
    func createTable() {
        let createTableString = "CREATE TABLE IF NOT EXISTS WishListDetails(productid INTEGER PRIMARY KEY AUTOINCREMENT,productname TEXT,productprice TEXT,productcategory TEXT,productDiscription TEXT,productRating TEXT,productImage BLOB );"
    ...
    func insert( productname:String, productprice:String,productcategory:String,productDiscription:String,productRating:String,image:UIImage){
        ...
        let insertStatementString = "INSERT INTO WishListDetails (productname, productprice, productcategory, productDiscription, productRating, productImage ) VALUES (?, ?, ?, ?, ?, ?);"
    ...
    func retrieveProducts(withCategory category: String) -> [WishListDetails] {
        var products: [WishListDetails] = []
        ...
        if sqlite3_prepare_v2(db, query, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                ...
                if let imageData = imageData {
                    ...
                    let product = WishListDetails(productname: productName, productprice: productPrice, productcategory: productCategory, productid: Int(productId), productDiscription: productDescription, productRating: productRating, image: image!)
    ...
    func retrieveProductsDetails() -> [WishListDetails] {
        var products: [WishListDetails] = []
        
        let query = "SELECT * FROM WishListDetails"
        var queryStatement: OpaquePointer?
    ...
    func read() -> [WishListDetails] {
        let queryStatementString = "SELECT * FROM WishListDetails;"
        var queryStatement: OpaquePointer? = nil
        var HomeProduct : [WishListDetails] = []
        ...
        if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                ...
                if let imageData = imageData {
                    ...
                    let image = UIImage(data: data)
                    // Use the image as needed
                    HomeProduct.append(WishListDetails(productname: productname, productprice: productprice, productcategory: productcategory, productid: Int(productid), productDiscription: productDiscription, productRating: productRating, image: image!))
                     print("Query Result:")
    ...
    func deleteByName(productname:String) {
        let deleteStatementString = "DELETE FROM WishListDetails WHERE productName = ?;"
